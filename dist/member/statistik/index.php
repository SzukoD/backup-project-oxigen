...
<section class="trainers-section py-5" id="pelatih" style="overflow:visible;">
  <div class="container" data-aos="zoom-in" data-aos-delay="100">
    <div class="text-center mb-5">
      <h2 class="fw-bold section-title">Jadwal Latihan Bersama Pelatih</h2>
      <p class="section-subtitle" style="color: white;">Pilih jadwal pelatih profesional sesuai minat dan waktu Anda</p>
    </div>
          ...
          <div class="col-12 col-sm-6 col-lg-4 d-flex justify-content-center">
            <div class="trainer-card text-center p-4 d-flex flex-column align-items-center"
              ...
              <p class="text-muted mb-2" style="font-size:0.95rem;">
                <?php echo htmlspecialchars($row['spesialisasi'] ?: 'Pelatih Umum'); ?>
              </p>

              <!-- Jadwal Ringkas -->
              <div class="text-start mt-3" style="font-size:0.9rem;">
                <p class="mb-1"><i class="bi bi-calendar-week text-danger me-2"></i>
                  <strong>Jadwal:</strong> <?php echo htmlspecialchars($row['jadwal_available'] ?: '-'); ?>
                </p>
                <p class="mb-1"><i class="bi bi-geo-alt-fill text-primary me-2"></i>
                  <strong>Tempat:</strong> <?php echo htmlspecialchars($row['tempat'] ?: 'Belum ditentukan'); ?>
                </p>
                <p class="mb-1"><i class="bi bi-people-fill text-success me-2"></i>
                  <strong>Kuota:</strong> <?php echo htmlspecialchars($row['kuota']); ?> peserta
                </p>
              </div>

              <div class="trainer-actions d-flex justify-content-center gap-3 mt-auto flex-wrap pt-3">
                <button class="btn btn-outline-dark px-4 py-2"
                 ...
                 <button class="btn px-4 py-2 btn-ikuti-jadwal" 
        style="background:#A0153E; color:white; border-radius:25px;"
        data-id="<?php echo $id_pelatih; ?>">
  <i class="bi bi-calendar-check me-2"></i>Ikuti Jadwal Ini
</button>
              </div>
            </div>
          </div>
                  ...
                  <div class="d-flex align-items-center gap-3 w-100">
                    ...
                    <div class="flex-grow-1">
                      ...
                      <div class="mt-2">
                        ...
                        <span class="badge bg-success px-3 py-2 rounded-pill ms-2">
                ...
                <div class="modal-body p-4">
                  <div class="row g-4">
                    ...
                    <div class="col-md-6">
                      <div class="info-card p-3 rounded" style="background:#f8f9fa;">
                        <h6 class="fw-bold mb-3" style="color:#A0153E;">
                          <i class="bi bi-calendar-week-fill me-2"></i>Jadwal & Tempat
                        </h6>
                        <p><i class="bi bi-clock-fill text-info me-2"></i><?php echo htmlspecialchars($row['jadwal_available'] ?: 'Belum diatur'); ?></p>
                        <p><i class="bi bi-geo-alt-fill text-danger me-2"></i><?php echo htmlspecialchars($row['tempat'] ?: 'Belum ditentukan'); ?></p>
                        <p><i class="bi bi-people-fill text-success me-2"></i>Kuota: <?php echo htmlspecialchars($row['kuota']); ?> peserta</p>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  <i class="bi bi-calendar-check me-2"></i>Ikuti Jadwal Ini
      ...
      <?php
        ...
        echo '<i class="bi bi-info-circle-fill me-2 fs-4"></i><br>Belum ada jadwal pelatih aktif.';
        echo '</div></div>';
      }
      ?>
    </div>
...
<div class="mt-5 container">
  <h3 class="text-center mb-4" style="color:#A0153E;">Latihan Offline yang Kamu Ikuti</h3>
  <div id="latihanDiikuti" class="row justify-content-center">
    <!-- Card akan diisi JS -->
  </div>
</div>
...
<script>
function loadLatihanDiikuti(){
  fetch('ambil_latihan_diikuti.php')
    .then(res=>res.json())
    .then(data=>{
      const container = document.getElementById('latihanDiikuti');
      container.innerHTML = '';
      if(data.length === 0){
        container.innerHTML = '<p class="text-center text-white">Belum mengikuti latihan offline mana pun.</p>';
        return;
      }
     data.forEach(item=>{
  const wrapper = document.createElement('div');
  wrapper.classList.add('col-md-4', 'card-latihan-wrapper');
  
  wrapper.innerHTML = `
    <div class="card-latihan">
     
      <h5>${item.nama_pelatih}</h5>
      <p>${item.spesialisasi || 'Pelatih Umum'}</p>
      <p><i class="bi bi-calendar-week me-1"></i> ${item.jadwal_available}</p>
      <p><i class="bi bi-geo-alt-fill me-1"></i> ${item.tempat || '-'}</p>
      <span class="badge-latihan">Sudah Diikuti</span>
    </div>
  `;
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.querySelectorAll('.btn-ikuti-jadwal').forEach(btn=>{
    btn.addEventListener('click', function(){
      const id_pelatih = this.dataset.id;

      fetch('ikuti_latihan.php',{
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded'},
        body:`id_pelatih=${id_pelatih}`
      })
      .then(res=>res.json())
      .then(data=>{
        if(data.status==='success'){
          Swal.fire({
            icon:'success',
            title:'Berhasil!',
            text:'Kamu berhasil mengikuti latihan ini.',
            timer:2000,
            showConfirmButton:false
          }).then(()=>{
            loadLatihanDiikuti();
            location.reload(); // bisa dihilangkan jika mau update kuota realtime tanpa reload
          });
        } else {
...
/* Kontainer grid */
/* Setiap card menjadi kolom responsif */
#latihanDiikuti .card-latihan-wrapper {
    display: flex;
    justify-content: center;
    margin-bottom: 20px;
}

#latihanDiikuti .card-latihan {
    background: linear-gradient(135deg, #A0153E, #FF4D4D);
    color: #fff;
    border-radius: 18px;
...
/* Foto pelatih */
#latihanDiikuti .card-latihan img {
    width: 100px;
    height: 100px;
    border-radius: 50%;
    object-fit: cover;
...
/* Badge status */
#latihanDiikuti .card-latihan .badge-latihan {
    display: inline-block;
    margin-top: 10px;
    padding: 5px 12px;
...
/* Efek kilauan hover */
#latihanDiikuti .card-latihan::before {
    content: '';
    position: absolute;
    top: -50%;
    left: -50%;
...
#latihanDiikuti .card-latihan:hover::before {
    transform: rotate(45deg) translateX(200%);
}

#latihanDiikuti .card-latihan:hover {
