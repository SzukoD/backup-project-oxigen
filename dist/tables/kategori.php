...
<div class="container-fluid mt-4">
  <div class="header-section d-flex justify-content-between align-items-center mb-3">
    <h3>🏋️ Data Kategori Latihan Gym</h3>
  ...
  <div class="card table-gym p-4 shadow-lg border-0 text-light rounded-4">
    <h5 class="fw-bold mb-3 text-white">
      <i class="bi bi-journal-bookmark-fill me-2 text-danger"></i>Daftar Kategori Latihan Gym
    </h5>
...
include '../../koneksi.php';
$no = 1;
$query = mysqli_query($conn, "SELECT * FROM kategori_latihan ORDER BY id_kategori DESC");
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $thumbPath = "../uploads/kategori/" . $row['gambar'];
    if (!empty($row['gambar']) && file_exists($thumbPath)) {
        echo "<img src='../uploads/kategori/{$row['gambar']}' class='rounded shadow' width='60' height='60'>";
    } else {
...
<!-- ====== MODAL EDIT KATEGORI LATIHAN ====== -->
<div class="modal fade" id="editKategoriModal" tabindex="-1" aria-labelledby="editKategoriModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="formEditKategori" enctype="multipart/form-data" method="POST">
        <input type="hidden" name="action" value="update">
        <input type="hidden" name="id_kategori" id="edit_id_kategori">
        <input type="hidden" name="gambar_lama" id="edit_gambar_lama">

        <div class="modal-header bg-warning text-dark">
          <h5 class="modal-title" id="editKategoriModalLabel">Edit Kategori Latihan</h5>
        ...
        <div class="modal-body">
          ...
          <div id="edit_gambar_preview" class="text-center my-2"></div>
        </div>
...
<script>
  ...
  // ✏️ EDIT KATEGORI LATIHAN
  document.querySelectorAll(".edit-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const data = JSON.parse(btn.dataset.program); // ambil data dari data-program
      ...
      if(data.gambar && data.gambar.trim() !== ""){
        preview.innerHTML = `<img src="../uploads/kategori/${data.gambar}" class="rounded shadow" width="80" height="80">`;
      } else {
        preview.innerHTML = `<span class="text-muted">Tidak ada gambar</span>`;
      }
  ...
  document.getElementById("formEditKategori").addEventListener("submit", async (e)=>{
    ...
    const fd = new FormData(form); // form data termasuk file

    try{
      const res = await fetch("../php/proses_kategori.php", { method: "POST", body: fd });
  ...
  //   btn.addEventListener("click", () => {
  //     const id_kategori = btn.dataset.id_kategori; // ambil dari data-id_kategori
  //     Swal.fire({
  //       title: "Hapus Kategori?",
  //       text: "Data kategori latihan akan dihapus permanen!",
  //       icon: "warning",
  ...
  //         fd.append("id_kategori", id_kategori); // harus sesuai PHP
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title" id="modalTambahProgramLabel">Tambah Kategori Latihan Gym</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form id="formTambahProgram" method="POST" enctype="multipart/form-data" action="../php/proses_kategori.php">
...
$(document).ready(function() {
  ...
  document.querySelectorAll(".edit-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const data = JSON.parse(btn.dataset.program); // data-program dari tombol
  ...
  // 🖼️ === PREVIEW GAMBAR (EDIT) ===
  const editGambarInput = document.getElementById("edit_gambar");
  if (editGambarInput) {
    editGambarInput.addEventListener("change", (e) => {
      const file = e.target.files[0];
      const preview = document.getElementById("edit_gambar_preview");
      preview.innerHTML = "";
      if (file) {
  ...
  // 💾 === SUBMIT EDIT KATEGORI LATIHAN ===
  const formEditKategori = document.getElementById("formEditKategori");
  if (formEditKategori) {
    formEditKategori.addEventListener("submit", async (e) => {
      e.preventDefault();
      const submitBtn = formEditKategori.querySelector("button[type='submit']");
      submitBtn.disabled = true;
      submitBtn.innerHTML = "Menyimpan...";

      const fd = new FormData(formEditKategori);
      ...
      try {
        ...
        if (result.trim() === "success") {
          Swal.fire({ 
            icon: "success", 
            title: "Berhasil!", 
            text: "Kategori latihan berhasil diperbarui.", 
            timer: 1800, 
            showConfirmButton: false 
          }).then(() => location.reload());
        } else {
  ...
  // ❌ === DELETE KATEGORI LATIHAN ===
  document.querySelectorAll(".delete-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const id_kategori = btn.dataset.id_kategori;
          ...
          try {
            ...
            if (text.trim() === "success") {
              Swal.fire({
                icon: "success",
                title: "Berhasil!",
                text: "Kategori latihan berhasil dihapus.",
                timer: 1500,
                showConfirmButton: false
              }).then(() => location.reload());
            } else {
